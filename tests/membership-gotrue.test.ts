...
test("GoTrue v2.177.0 actual admin deletion refuses atomically and explicit same-ID retry completes",{skip:!url||!binary},async()=>{
    ...
    try {
        ...
        for(const name of ["20260907212654_create_patreon_links.sql","20260907220000_patreon_website_roles.sql","20260907230000_atomic_live_console_assignments.sql","202609080002_membership_onboarding.sql","202609080003_membership_role_locking.sql","20260908030000_patreon_event_reconciliation.sql","20260910200000_membership_receipt_claims.sql","202609110001_edge_owned_link_commit.sql"]) await db.query(await readFile(`supabase/migrations/${name}`,"utf8"));
