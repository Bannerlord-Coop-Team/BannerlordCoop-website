...
The protected `/admin/control-plane` page is the administrative surface for managed hosting. It reads fleet/server/job/release/audit state and submits the complete supported administrator operation set through the `control-plane-admin` Supabase Edge Function. The page and function require the freshly verified Supabase Auth `app_metadata.role === "Admin"`; email bootstrap access alone does not authorize this surface. Discord linkage is not required for an administrator. The function relays only the bearer token and unchanged typed request to the Oracle control-plane web adapter, which independently reauthorizes the account. Unlinked-account support requires coordinated backend, Edge, and website release; customer ownership and targets remain Discord-based. The website does not query the private control-plane schema or talk directly to OVH, runners, containers, or object storage. See `docs/control-plane-admin.md` and the control-plane repository's `docs/managed-hosting/web-admin.md`.
