...
vi.mock("@/app/account/actions", () => ({ resolveAccountLink: vi.fn(), completePatreonAccount: vi.fn(), confirmDiscordAccount: vi.fn(), linkDiscordAccount: vi.fn(), linkPatreonAccount: vi.fn(), unlinkPatreonAccount: vi.fn() }));
...
vi.mock("@/app/lib/supabase/server", () => ({ getSupabaseServerClient: async () => ({
    ...
    functions: { invoke: async (_name: string, options: { body: { provider: string } }) => ({ data: { accountId, provider: options.body.provider, state: "none" }, error: null }) },
}) }));
import AccountPage from "./page";

afterEach(() => vi.clearAllMocks());
async function render(membership = EMPTY_MEMBERSHIP) {
    mocks.status.mockResolvedValue({ hasDiscord: true, membership });
    const element = document.createElement("div");
    element.innerHTML = renderToStaticMarkup(await AccountPage({ searchParams: Promise.resolve({}) }));
    return element;
}
