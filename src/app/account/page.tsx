    ...
    return <><Navbar /><main className="min-h-[70svh] bg-background"><section className="site-container py-16 sm:py-20" aria-labelledby="account-heading">
        ...
        {(["discord", "patreon"] as const).map(provider => {
            ...
            return <div key={provider} className="mt-6 border border-gold/30 p-5" aria-label={`${label} confirmation recovery`}>
                ...
                <form action={resolveAccountLink}><input type="hidden" name="provider" value={provider} /><input type="hidden" name="operationId" value={pending.operationId} /><button className="mt-3 underline" type="submit">{["committed", "resolved"].includes(pending.state) ? `Recover ${label} result` : pending.state === "expired" ? `Resolve expired ${label} attempt` : `Cancel pending ${label} attempt`}</button></form>
            </div>;
        })}
        <div id="link-account" className="mt-8 space-y-5">
            {status && !status.hasDiscord && recovery.discord?.state === "none" && <form action={linkDiscordAccount}><input type="hidden" name="returnPath" value="/account" /><p>Confirm that you want to connect Discord to this signed-in account.</p><button type="submit" className="mt-3 underline">Confirm and connect Discord</button></form>}
