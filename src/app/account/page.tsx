...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ patreon?: string; discord?: string }> }) {
    ...
    return <><Navbar /><main className="min-h-[70svh] bg-background"><section className="site-container py-16 sm:py-20" aria-labelledby="account-heading">
        ...
        {(params.patreon === "rate_limited" || params.discord === "rate_limited") && <p role="alert" className="mt-4">Too many account changes or synchronization is still catching up. Wait at least 10 minutes, then retry the same pending confirmation. Do not restart authorization. Unlink and existing server management remain available.</p>}
