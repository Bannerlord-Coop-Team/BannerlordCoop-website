...
export function forceUpdateServerAction(serverField: AdminActionField) {
    return {
        ...
        title: "Force Update",
        description: "Resolve the latest GHCR image for this server's configured Stable or Nightly channel to an immutable digest, then reinstall it—even when that digest is already installed. Replaces the existing build pin; clear the pin afterward to resume catalog maintenance. Image compatibility is unverified. Running games restart; stopped games stay stopped.",
        destructive: true,
