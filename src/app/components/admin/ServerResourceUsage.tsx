...
export function ServerResourceUsage({ resources }: { resources?: HostingServerResources | null }) {
    ...
    return <dl className="mt-2 space-y-1 text-xs text-foreground-muted" aria-label="Server resource usage">
        ...
        <div className="text-[0.62rem] text-foreground-dim"><dt className="inline">Observed </dt><dd className="inline"><LocalDateTime value={resources.observedAt} /></dd></div>
        <div className="text-[0.62rem] text-foreground-dim"><dt className="sr-only">Scope</dt><dd>Includes game and controller overhead.</dd></div>
    </dl>;
}
