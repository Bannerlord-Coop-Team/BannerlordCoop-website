...
export function ServerRow({ server }: ServerRowProps) {
    ...
    return (
        <tr className="group border-b border-white/10 transition-colors duration-300 hover:bg-white/2.5">
            ...
            <td className="whitespace-nowrap px-6 py-5 font-sans text-sm text-foreground-muted">
                {server.region}
            </td>
            ...
            <td className="whitespace-nowrap px-6 py-5 font-label text-sm font-semibold tabular-nums text-foreground">
                {server.warriors}{" "}
                <span className="text-foreground-dim">/ {server.maxWarriors}</span>
            </td>
            <td className={`whitespace-nowrap px-6 py-5 font-label text-sm font-semibold tabular-nums ${getPingColor(server.ping)}`}>
