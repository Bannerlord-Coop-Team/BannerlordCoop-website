...
const OPERATION_EXPLANATIONS: Record<string, string> = {
    ...
    "onboard-vps-host": "Verifies one already-purchased OVH VPS from authenticated account inventory, automatically pins its SSH identity, uses the preinstalled fleet key, hardens the host, installs and activates every isolated managed-runner slot through private mTLS routes, and publishes capacity only after health proof. It never orders, renews, or cancels the VPS.",
