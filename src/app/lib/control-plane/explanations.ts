...
const OPERATION_EXPLANATIONS: Record<string, string> = {
    ...
    "onboard-vps-host": "Verifies one already-purchased OVH VPS from authenticated account inventory, automatically pins the Ed25519 SSH identity presented by its provider-derived public IP on first contact, installs and activates every isolated managed-runner slot through private mTLS routes, and publishes capacity only after health proof. It never orders, renews, or cancels the VPS, and accepts no browser-supplied host identity or topology.",
