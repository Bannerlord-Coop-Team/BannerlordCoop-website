...
function OperationsView({ data, discordUsers }: { data: OperationsData; discordUsers: DiscordUserSummary[] }) {
    ...
    const cards: Array<{ group: string; operation: string; title: string; description: string; fields: AdminActionField[]; destructive?: boolean }> = [
        { group: "Fleet", operation: "onboard-vps-host", title: "Onboard existing OVH VPS", description: "Select one already-purchased VPS discovered from the authenticated OVH account. The control plane derives its service identity, reviewed location, vCPU capacity, and public IPv4, pins the Ed25519 SSH identity it presents on first contact, installs every managed runner slot, establishes private mTLS routes, and publishes capacity only after health checks. It never buys, renews, or cancels a VPS.", fields: [
            { name: "serviceName", label: "Available OVH VPS", kind: "select", required: true, options: availableVpsOptions, defaultValue: availableVpsOptions.length === 1 ? availableVpsOptions[0]!.value : "", help: "Only VPS products discovered in the authenticated OVH account and not already registered are shown. The VPS must already authorize the control plane's fleet operator public key for the default Debian account. Everything else, including topology discovery and first-contact host-key pinning, is automatic." },
        ] },
