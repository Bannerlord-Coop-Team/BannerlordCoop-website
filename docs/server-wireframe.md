...
Public route: `/servers/wireframe`. Append this path to the Cloudflare preview origin after deploying this branch. No sign-in, environment variables or server connection are needed by this route. The authenticated `/servers/[serverId]` page now uses the wireframe's four-workspace layout through `ServerManagementWorkspace`. It retains real identity, authorization, lifecycle controls and live console connections. Managed backups and save/config transfers occupy their own tabs; authorized access management is in Settings. The compact Public/Private dropdown stays in the header, and managed game state, lifecycle and release channel stay visible below the connection address. Unconnected features remain visible but disabled, including the illustrative command picker. Placeholder servers no longer simulate successful operations. Tabs keep controls mounted so connections, drafts and pending work survive navigation. The live console supports pausing auto-scroll. For control-plane-managed servers, Download logs retrieves the latest `.log` file (by last-modified time) from `<data-dir>/logs`, preserving its filename, without requiring a live console connection. The download uses the authenticated `my-server-latest-log` control-plane operation and is limited to 100 MiB. Binary responses stream through the edge proxy directly to the browser, avoiding base64 and Next server-action buffering. Legacy live-only and preview servers have log downloads disabled. The public route remains the fictional design demo described below.
