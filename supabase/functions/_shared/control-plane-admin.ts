...
const SUPABASE_ACCOUNT_ID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/u;
...
export function createControlPlaneAdminHandler(options: ControlPlaneAdminHandlerOptions) {
    ...
    return async (request: Request): Promise<Response> => {
        ...
        if (typeof user.id !== "string" || !SUPABASE_ACCOUNT_ID.test(user.id)) {
            return envelopeError(409, requestId, "identity_unavailable", "A verified Supabase account is required.", false, cors);
        }
