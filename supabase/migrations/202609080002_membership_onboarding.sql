...
begin
 ...
 if h.discord_user_id is distinct from p_discord_user_id or h.patreon_user_id is distinct from current_patreon or h.link_state is distinct from state then
  ...
  insert into public.membership_outbox(account_id,revision) values(h.account_id,h.revision);
 end if;
...
begin
 ...
 insert into public.membership_outbox(account_id,revision) select account_id,revision from public.membership_heads where account_id=p_account_id;
...
begin
 ...
 -- An explicit unlink also fences in-flight completion while already unlinked.
 update public.membership_heads set patreon_user_id=null,link_state='unlinked',link_generation=link_generation+1,revision=revision+1,evidence=public.membership_empty_evidence() where account_id=p_account_id;
 insert into public.membership_outbox(account_id,revision) select account_id,revision from public.membership_heads where account_id=p_account_id;
 return (select public.membership_snapshot_json(h) from public.membership_heads h where account_id=p_account_id);
end $$;
...
begin
 snapshot := public.membership_fence(p_account_id,p_discord_user_id,false);
 select exists(select 1 from public.membership_outbox where account_id=p_account_id and revision=(snapshot->>'revision')::bigint and receipt_id is null) into pending;
